...
def get_rank_and_color(level):
    if level <= 5:
        ...
        return "Cadet Officer", Colors.UNCOMMON
    elif level <= 30:
        return "Cadet Commander", Colors.RARE
    elif level <= 50:
        return "Starfleet Captain", Colors.EPIC
    else:
        return "Fleet Admiral", Colors.LEGENDARY
...
# Impossible
    ...
    if diff_val <= 3:
        ...
        difficulty_name = "Skilled"
        diff_color = Colors.EPIC
    elif diff_val <= 80:
        difficulty_name = "Brutal"
    ...
    if diff_val <= 5:
        ...
        rarities = ["COMMON", "UNCOMMON"] if rarity_roll < 0.3 else ["UNCOMMON"]
    elif diff_val <= 30:
        rarities = ["UNCOMMON"] if rarity_roll < 0.25 else (["RARE"] if rarity_roll > 0.8 else ["UNCOMMON", "RARE"])
    elif diff_val <= 50:
        rarities = ["RARE"] if rarity_roll < 0.2 else (["EPIC"] if rarity_roll > 0.85 else ["RARE", "EPIC"])
    elif diff_val <= 80:
        rarities = ["EPIC"] if rarity_roll < 0.15 else (
            ["LEGENDARY"] if rarity_roll > 0.9 else ["EPIC", "LEGENDARY"])
    else:
        rarities = ["LEGENDARY"] if rarity_roll < 0.3 else ["LEGENDARY", "EPIC"]
...
def main():
    ...
    while True:
        try:
            ...
            if level >= 101:
                print(f"{Colors.FAIL}Error: Cadet Level must be a positive integer (>= 100).{Colors.ENDC}")
                continue
